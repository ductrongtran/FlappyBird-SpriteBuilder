...
// The interface for CGPointObject is hidden in CCParallaxNode.m
// CCParallaxNode is a class included in Cocos2D
// We include this outline for the CGPointObject class so that Xcode
// knows it exists and it doesn't throw any errors when
// we try to access its offset property later on
...
@implementation MainScene {
    // Parallax ratios for the clouds and bushes. Ratio (1, 1)
    // means that the object moves normally with the layer in
    // both the x and the y direction. We will move our layer in
    // the x direction so we will only modify the x value of the
    // ratio. The clouds are farther away than the bushes so they
    // should be moving slower than the bushes. We will give the
    // clouds a parallax ratio with a lower x value.
...
- (void)didLoadFromCCB {
    ...
    // The bush ratio is larger than the cloud's
